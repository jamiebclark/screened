...
describe("normalizeLetterboxdActivityUrl", () => {
  it("normalizes https letterboxd.com links", () => {
    expect(
      normalizeLetterboxdActivityUrl(
        "https://letterboxd.com/alice/film/inception/",
      ),
    ).toBe("https://letterboxd.com/alice/film/inception/");
  });

  it("upgrades http to https", () => {
    expect(
      normalizeLetterboxdActivityUrl("http://letterboxd.com/alice/film/foo/"),
    ).toBe("https://letterboxd.com/alice/film/foo/");
  });
  ...
  it("allows subdomains of letterboxd.com", () => {
    expect(
      normalizeLetterboxdActivityUrl("https://www.letterboxd.com/user/film/x/"),
    ).toBe("https://www.letterboxd.com/user/film/x/");
  });

  it("rejects other hosts", () => {
    expect(
      normalizeLetterboxdActivityUrl("https://evil.com/letterboxd.com/"),
    ).toBeNull();
    expect(
      normalizeLetterboxdActivityUrl("https://notletterboxd.com/foo"),
    ).toBeNull();
  });
