...
import { defaultPickerState, isPickerState, withScoringDefaults, type PickerRoomState } from "@/lib/picker-room-state";
...
export default async function PickPage({ searchParams }: PageProps) {
  ...
  const [currentUser, savedPreferences, roomRow, plexConnection] = await Promise.all([
  ...
  if (roomRow && isPickerState(roomRow.state)) {
    ...
    if (!st.participants.some((p) => p.id === currentUser!.id)) {
      initialRoomState = withScoringDefaults({ ...st, participants: [currentUser!, ...st.participants] });
    } else {
      initialRoomState = st;
    }
  } else {
    ...
    <div className="mx-auto max-w-7xl px-4 py-8">
      <div className="mb-8">
        ...
        <p className="text-muted-foreground mt-1">
          Tell us what you&apos;re in the mood for and we&apos;ll find the best match from your library.
        </p>
      </div>
      <PickSession
