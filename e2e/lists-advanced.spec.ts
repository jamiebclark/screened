...
test.describe("Lists - Advanced", () => {
  test("private list is inaccessible to non-members without slug", async ({
    page,
    browser,
  }) => {
  ...
  test("radarr endpoint returns movie list JSON", async ({ page }) => {
    ...
    const data = (await radarr.json()) as Array<{
      tmdbId: number;
      title: string;
    }>;
  ...
  test("radarr endpoint works for private list with correct token", async ({
    page,
  }) => {
    const res = await page.request.post("/api/lists", {
  ...
  test("watchlist radarr endpoint returns movie JSON with token", async ({
    page,
  }) => {
    ...
    const radarr = await page.request.get(
      `/api/user/radarr/watchlist?token=${token}`,
    );
    expect(radarr.ok()).toBeTruthy();
    const data = (await radarr.json()) as Array<{
      tmdbId: number;
